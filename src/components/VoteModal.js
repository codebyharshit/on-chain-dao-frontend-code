...
const VoteModal = (props) => {
    const { setShowModal, voteFor, proposalId } = props;
    const handleSubmit = async() => {
      const vote = await contract.castVote(proposalId, voteFor);
      const txn = await vote.wait(1);
      setShowModal(false);
    };
    return (
              ...
              <div className="mx-auto max-w-2xl mb-5 text-lg font-medium rounded-lg border border-gray-600 text-white">
                <div className="px-6 pt-6 pb-8" style={{ width: 450 }}>
                  ...
                  <div className="w-full flex justify-between">
                    ...
                    <span className="text-base">Variable Tokens</span>
                  </div>
                </div>
              </div>
